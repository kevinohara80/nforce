...
function Subscription(opts, client) {
  ...
  // check to see if there's a replayId set on the client already
  if (typeof opts.replayId !== 'undefined' && opts.replayId !== null && client.getReplayId(this._topic)) {
    this._replayId = client.getReplayId(this._topic);
  } else {
    this._replayId = getReplayId(opts.replayId);
  }
  
  // ensure a replayId is set on the client
...
Client.prototype.setReplayMap = function (replayMap) {
  // TODO: convert topic props to lowercase
  this._replayMap = replayMap;
}
