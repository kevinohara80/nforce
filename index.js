...
const AUTH_REVOKE = 'https://login.salesforce.com/services/oauth2/revoke';
const TEST_AUTH_REVOKE = 'https://test.salesforce.com/services/oauth2/revoke';
...
const SAML_ASSERTION = 'urn:oasis:names:tc:SAML:2.0:profiles:SSO:browser';
...
const Connection = function(opts) {
    ...
    opts.apiVersion = opts.apiVersion
        .toString()
        .toLowerCase()
        .replace('v', '')
        .replace('.0', '');
...
Connection.prototype.authenticate = function(data, callback) {
    const self = this;
    const opts = _.defaults(this._getOpts(data, callback), {
        executeOnRefresh: false,
        oauth: {}
    });
    const resolver = promises.createResolver(opts.callback);
    ...
    if (opts.code) {
        ...
        bopts.assertion_type = SAML_ASSERTION;
...
Connection.prototype.refreshToken = function(data, callback) {
    ...
    const opts = this._getOpts(data, callback, {
        defaults: {
    ...
    if (!opts.oauth.refresh_token && (opts.oauth.assertion || opts.assertion)) {
        ...
        refreshOpts.assertion_type = SAML_ASSERTION;
...
Connection.prototype.revokeToken = function(data, callback) {
    ...
    if (this.environment === 'sandbox') {
        opts.uri = TEST_AUTH_REVOKE;
    } else {
        opts.uri = AUTH_REVOKE;
    }
...
Connection.prototype.getVersions = function(callback) {
    const opts = this._getOpts(null, callback);
...
Connection.prototype.getAttachmentBody = function(data, callback) {
    const opts = this._getOpts(data, callback);
    const id = opts.sobject ? sobject.getId() : opts.id;
...
Connection.prototype._apiAuthRequest = function(opts, callback) {
    ...
    const resolver = opts._resolver || promises.createResolver(callback);
...
Connection.prototype._apiRequest = function(opts, callback) {
    ...
    if (opts.uri) {
        ...
        ropts.uri = [opts.oauth.instance_url, '/services/data/', this.apiVersion, opts.resource].join('');
    }
